		...
		{	
			...
			if ($res = $conn->query($sql))
			{
				if($resKey = $conn->query($getKey))
				{
					...
					echo '<h3 style="margin-bottom: -10px;">Edycja rekordów w tabeli <strong>'.$tableName.'</strong> <span class="badge">'.mysqli_num_rows($res).'</span></h3>';
					echo "<hr/>";
					...
					{   //Creates a loop to loop through results
						...
						echo '<td><a href="edytuj_rekord.php" class="btn btn-warning" style="width:100%; float:none;" onclick="saveDataToEdit(this)" >Edytuj</a></td>';
						echo "</tr>";
					}
